...
Below is the command that I ended up splicing together.<br>
```sudo naabu -host 10.1.0.0/24 -nmap-cli "nmap -sV -oX scan.xml" && searchsploit --nmap scan.xml```<br>
...
I remember reading about how the /etc/shadow file contains passwords and the /etc/passwd file only contains username, but an attack vector we found in this class was to use the openssl command on our attack machine.<br>
```openssl passwd password```<br>
This command creates a hash from the password entered in the same format of the shadow file hashes. Then, if the passwd file is writable by your user, you can paste the password over the x in the root user entry.<br>
